...
\icmltitlerunning{Training Linear Models with End-to-End Low Precision}

\begin{document}

\twocolumn[
\icmltitle{Training Linear Models with End-to-End Low Precision,\\
...
We validate that if we use large mini-batch size, the SGD will be more ``sensitive'' to the precision. In theory, as inequation~\ref{neq:decompose} suggests, the variance are composed by two terms:
the first one is from the full stochastic gradient and the second one is from quantization. Our assumption is that,
if we reduce the first term by using mini-batch, then the total variance is more ``sensitive'' to the second term. 
Thus, to achieve a comparable convergence rate, We need to make sure that the second term introduced by quantization isn't too big. However, figure~\ref{fig:minibatch} shows that, if we use larger mini-batch size (256), we need more epochs
