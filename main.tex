...
\icmltitlerunning{ZipML: Training Linear Models with End-to-End Low Precision}

\begin{document}

\twocolumn[
\icmltitle{ZipML: Training Linear Models with End-to-End Low Precision,\\
and a Little Bit of Deep Learning}


\begin{icmlauthorlist}
\icmlauthor{Hantian Zhang}{eth}
\icmlauthor{Jerry Li}{mit}
\icmlauthor{Kaan Kara}{eth}
\icmlauthor{Dan Alistarh}{eth,ist}
\icmlauthor{Ji Liu}{roc}
...
\icmlaffiliation{roc}{University of Rochester, USA}
\icmlaffiliation{ist}{IST Austria, Austria}

\icmlcorrespondingauthor{Hantian Zhang}{hantian.zhang@inf.ethz.ch}
\icmlcorrespondingauthor{Ce Zhang}{ce.zhang@inf.ethz.ch}
    \vskip 0.3in
]
...
We mainly focus on linear models, and the answer is yes for linear models. We develop a simple 
framework called ZipML based on one simple but novel strategy called double sampling. 
Our ZipML framework is able 
...
In this paper, we develop ZipML, a general 
framework to answer this question, and
present results
 obtained in the context of this ZipML framework. 
...
are used symmetrically, we only need to send a number representing the number�� of times  the lower quantization level has been chosen among all the sampling trials. 
...
In the full version of this paper~\cite{zhang2016zipml}, we present an algorithm which, given $\Omega$ and $k$, provides a split using at most $4 k$ intervals, which guarantees a $2$-approximation of the optimal variance for $k$ intervals, using $O( N \log N )$ time. This 
...
decisions can be found in ~\citet{kara2017fpga}.
...
FPGA platform. The detailed 
implementation is described in ~\cite{kara2017fpga}.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\columnwidth]{minibatchs} 
%\vspace{-1em}
...
\paragraph{Impact of Using Mini-Batch.}


We validate that if we use large mini-batch size, the SGD will be more ``sensitive'' to the precision. Equation~\ref{eqn:varbound} suggests that, as we increase batch size, the variance term corresponding to input quantization may start to dominate the variance of the stochastic gradient. However, in practice and for reasonable parameter settings, we found this does not occur: convergence trends for small batch size, e.g. 1, are the same as for larger sizes, e.g. 256.
...
We show that ZipML, a relatively simple stochastic quantization framework can achieve this for linear models. 
...
In the full version on this paper, we examine this problem, and find that our framework can be generalized to non-linear settings, and that in practice 8-bit is sufficient to achieve 
