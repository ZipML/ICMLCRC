...
In the full version of this paper, we present an algorithm which, given $\Omega$ and $k$, provides a split using at most $4 k$ intervals, which guarantees a $2$-approximation of the optimal variance for $k$ intervals, using $O( N \log N )$ time. This 
is a new variant of the algorithm by (Acharya et al., 2015)\todo{proper citation} for the histogram recovery problem. 
...
The additional material\todo{Add more results, and pointer to full version of the paper} contains (1) several other datasets, 
